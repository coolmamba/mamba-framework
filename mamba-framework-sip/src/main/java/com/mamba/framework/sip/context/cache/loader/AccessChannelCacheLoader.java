...
public class AccessChannelCacheLoader extends AbstractCacheLoader<String, AccessChannel> {
	...
	public Map<String, AccessChannel> data() {
		...
		List<AccessChannelCacheProvider> providers = getProviders(AccessChannelCacheProvider.class);
		for (int i = 0; null != providers && i < providers.size(); i++) {
			AccessChannelCacheProvider provider = providers.get(i);
			if (null != provider) {
				List<AccessChannel> channels = provider.provide();
